...
if (isset($_POST['giris'])){
  ...
  $sorgu = $vtabani-> query("SELECT * FROM users 
  WHERE BINARY user_mail = '$eposta' 
  and BINARY user_pass = '$sifre' ");
if ($sorgu->num_rows <= 0){
  $error = "hatalı";
  $vtabani->close();
} else {
  $sorguPerm = $vtabani-> query("SELECT user_perm FROM users 
  WHERE BINARY user_mail = '$eposta' 
  and BINARY user_pass = '$sifre' ");
  
  $sorguName = $vtabani-> query("SELECT user_name FROM users 
  WHERE BINARY user_mail = '$eposta' 
  and BINARY user_pass = '$sifre' ");

  $kAdi = mysqli_fetch_assoc($sorguName);
  $kPerm = mysqli_fetch_assoc($sorguPerm);
  $_SESSION['giris'] = true ;
  $_SESSION['login_time_stamp'] = time() ;
  $_SESSION['yetki'] = $kPerm['user_perm'] ;
...
<html>
  ...
  <body>
    ...
    if(!empty($error)){
      ...
      <div class="message"> Yanlış E-Posta yada Şifre girdiniz. </div>
  ...
  <script src="giris.js?v=0.03"></script>
</html>
