<?php
    ...
    $check_query = "SELECT * FROM courseschedule WHERE course_id = ? AND program_id = ? AND lecturer_id = ? AND classroom_id = ? AND course_start = ? AND course_end = ? AND course_day = ? and cs_id != ?";
    $check_stmt = $vtabani->prepare($check_query);
    $check_stmt->bind_param("iiiisssi", $dersAdi, $programAdi, $ogrAdi, $derslikAdi, $baslangicSaati, $bitisSaati, $gunAdi, $cs_id);
    $check_stmt->execute();
    ...
    if ($check_result->num_rows > 0) {
        ...
        $clash_stmt->bind_param("isiiiiiiiiiiiiiissiiiiiiiiiiiiiiiii",
            $cs_id,          // i 
            $gunAdi,         // s
            $derslikAdi,     // i
            $baslangicSaati, // i
        ...
        if ($clash_result->num_rows > 0) {
            ...
            if ($clash_row['classroom_id'] == $derslikAdi) {
                ...
                echo "Program hata";
                    ...
                    $sorgu = "UPDATE courseschedule 
                    SET timetable_id = ?, course_id = ?, program_id = ?, lecturer_id = ?, classroom_id = ?, course_start = ?, course_end = ?, course_day = ? 
                    WHERE cs_id = ?";
                    $hazirla = $vtabani->prepare($sorgu);
                    $hazirla->bind_param("iiiiiiisi", $timetable_id, $dersAdi, $programAdi, $ogrAdi, $derslikAdi, $baslangicSaati, $bitisSaati, $gunAdi, $cs_id);
                    $hazirla->execute();
                    echo 'Editleme işlemi tamamlandı.';
                }   
    ...
    //editlenmiş kartı gösterme kısmı
